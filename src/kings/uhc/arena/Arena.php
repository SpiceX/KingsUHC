...
class Arena implements Listener
{
	...
	{
		if (!$this->data["enabled"]) {
			$player->sendMessage("§6uhc » §7Arena is under setup!");
			return;
		}

		if (count($this->players) >= $this->data["slots"]) {
			$player->sendMessage("§6uhc » §7Arena is full!");
			return;
		}

		if ($this->inGame($player)) {
			$player->sendMessage("§6uhc » §7You are already in game!");
			return;
		}

		if ($this->phase != self::PHASE_LOBBY) {
			$player->sendMessage("§6uhc » §7This game has already started!");
			return;
		}
		...
		//$player->teleport(Position::fromObject(Vector3::fromString($this->data["deathmatch"]), $this->level));
		...
		$this->broadcastMessage("§6uhc » §7Player {$player->getName()} joined! §7[" . count($this->players) . "/{$this->data["slots"]}]");
	}
	...
	{
		$players = [];
		$centerX = explode(',', $this->data['deathmatch'])[0];
		$centerZ = explode(',', $this->data['deathmatch'])[2];
		foreach ($this->players as $player) {
			$players[$player->getName()] = $player;
			$player->teleport($this->level->getSafeSpawn(new Vector3($centerX + mt_rand(0, 150), 90, $centerZ + mt_rand(0, 150))));
			$player->setGamemode(Player::SURVIVAL);
			$this->bossbar->showTo($player);
			$player->getInventory()->addItem(Item::get(Item::STONE_AXE));
			$player->getInventory()->addItem(Item::get(Item::STONE_PICKAXE));
			$player->getInventory()->addItem(Item::get(Item::STONE_SHOVEL));
		}
		...
		$this->broadcastMessage("§6uhc » §aPvP will be enabled in 6 minutes.", self::MSG_MESSAGE);
	}
	...
	public function startRestart(bool $winner = true)
	{
		...
		if ($winner) {
			...
			$this->plugin->getServer()->broadcastMessage("§6uhc » §7No winners at {$this->level->getFolderName()}!");
		}
		$this->phase = self::PHASE_RESTART;
	}
	...
	public function startDeathmatch()
	{
		if ($this->deathmatch) {
			...
			$this->broadcastMessage("§4Deathmatch", Arena::MSG_TITLE, "§eFight!");
		}
	}
	...
	/**
	 * Shows the edge limit to all players
	 */
	public function broadcastBorder()
	{
		foreach ($this->players as $player) {
			$player->sendTip(PluginUtils::centerText("§6X: §7{$player->getX()}   §6Z: §7{$player->getZ()}\n" .
				"§6borderX: §7{$this->maxX}   §6borderZ: §7{$this->maxZ}"));
		}
	}
	...
	public function onDeath(PlayerDeathEvent $event)
	{
		...
		$this->broadcastMessage("§6uhc » §7{$this->plugin->getServer()->getLanguage()->translate($event->getDeathMessage())} §7[" . count($this->players) . "/{$this->data["slots"]}]");
	...
	public function disconnectPlayer(Player $player, string $quitMsg = "", bool $death = false)
	{
		...
		if (!$death) {
			$this->broadcastMessage("§6uhc » §7 Player {$player->getName()} left the game. §7[" . count($this->players) . "/{$this->data["slots"]}]");
		}

		if ($quitMsg != "") {
			$player->sendMessage("§6uhc » §7$quitMsg");
		}
	}
	...
	public function onBlockBreak(BlockBreakEvent $event)
	{
		...
		switch ($event->getPlayer()->getInventory()->getItemInHand()->getId()) {
			...
			case Item::DIAMOND_AXE:
				$damage = $this->destroyTree($player, $event->getBlock());
				if ($damage) {
	 ...
	 * @return int
	 */
	private function destroyTree(Player $player, Block $block)
	{
		$damage = 0;
